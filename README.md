...
- ★ | [Agentic Applications 實戰 Evals: 從 Traces, Failure Taxonomy 到 Experiments](2026-05-14-hands-on-agent-evals-Xfl50508LZM.md)
  以 financial analysis agent 建立完整 eval workflow, 涵蓋 trace 閱讀, 失敗分類, code eval, LLM judge, meta-evaluation, golden dataset 與 controlled experiments.
...
這個 repository 採用"先檢查來源責任, 再檢查內容證據"的雙層篩選機制. 核心不是講者的名氣, 而是講者是否接近事實, 是否必須為系統結果負責, 以及內容是否留下可驗證, 可操作的證據.

官方技術團隊對產品內部設計的一手說明, 以及 production owner 對失敗經驗與工程取捨的復盤, 通常具有較高可信度. 相較之下, KOL 觀點或新聞式摘要不必直接承擔系統結果, 若又缺少案例, 數字或實作細節, 收錄價值通常較低.

### 第一層: 來源責任

先問:

> 講者是否直接參與這套系統, 並需要為結果負責?

可以依可信度分成四級:

| 等級 | 來源類型 | 例子 | 預設處理 |
| --- | --- | --- | --- |
| A | 官方技術團隊的一手設計說明 | OpenAI, Anthropic 工程師解釋內部架構 | 優先收錄 |
| A | Production owner 的實戰復盤 | 實際維運系統的工程師分享 incident, 限制與取捨 | 優先收錄 |
| B | 有直接操作證據的第三方實作者 | 開源維護者, 企業導入者, 可重現 benchmark 作者 | 證據充分才收錄 |
| C | 教學型內容 | 技術課程, 工具操作, 概念入門 | 僅收錄適合補足知識結構者 |
| D | KOL 觀點或新聞摘要 | 趨勢評論, 產品新聞轉述, 排行榜 | 原則上不收錄 |

這裡的"官方"不是自動等於可信. 官方對產品事實通常是一手來源, 但對市場優勢, 未來預測及競品比較仍有宣傳誘因. 因此要區分:

- 官方說"系統如何設計", 一手證據價值高.
- 官方說"我們的方法最好", 仍需外部證據.
- 工程師說"我們遇到什麼事故", 經驗可信度高.
- 工程師說"所有公司都應該這樣做", 泛化程度需要另外評估.

### 第二層: 內容證據

即使來源合格, 內容仍要回答:

> 影片有沒有提供足以驗證, 重現或採取行動的資訊?

可以檢查六種高價值訊號:

1. 具體系統

   說明實際架構, 元件, 資料流, 控制流程或部署環境.

2. 具體問題

   描述發生了什麼錯誤, 限制, 事故或效能瓶頸.

3. 具體證據

   提供數字, trace, benchmark, 成本, 延遲, 錯誤率或使用規模.

4. 決策取捨

   解釋為何選 A 而非 B, 以及這個選擇犧牲了什麼.

5. 驗證方法

   說明測試, eval, 監控, rollback 或 production readiness 的判準.

6. 適用邊界

   清楚指出方法何時有效, 何時失敗, 需要哪些前提.

如果影片只說"context 很重要", 價值很低. 若它進一步說明 context 放在哪裡, 如何選取, token budget 如何分配, 失敗時 trace 長什麼樣, 才形成可用知識.

### 收錄門檻與評分方式

不要只靠總分, 因為一支製作品質很高的新聞摘要可能得到不錯分數, 卻仍缺少一手證據.
...
- 講者是系統設計者, 維運者或事故責任人.
- 內容包含可驗證的實作, 資料, 程式碼, 架構或失敗案例.
- 是必要的官方規格, 文件或產品行為說明.
- 是高品質基礎教學, 且能補足 repository 明確缺口.

以下情況直接排除:

- 只是轉述另一篇文章或產品發布.
- 全程只有趨勢判斷與抽象主張.
- 標題承諾實戰, 內容卻沒有系統, 案例或數字.
- 無法辨認講者與原始資料來源.
- 大量內容是未揭露依據的 AI 生成摘要.
- 與現有筆記高度重複, 沒有新增證據或視角.
...
| 維度 | 0 分 | 1 分 | 2 分 |
| --- | --- | --- | --- |
| 一手程度 | 純轉述 | 有間接參與 | 設計者或 production owner |
| 責任關係 | 無需承擔結果 | 有專業聲譽成本 | 對系統結果直接負責 |
| 具體程度 | 抽象觀點 | 有例子 | 有架構, 流程或程式碼 |
| 證據品質 | 無證據 | 個別案例 | 數字, trace, eval 或可重現結果 |
| 失敗資訊 | 只談成功 | 提到限制 | 詳述失敗, 原因與修正 |
| 取捨分析 | 宣稱最佳解 | 比較選項 | 清楚說明成本與適用邊界 |
| 可操作性 | 無法採用 | 有原則 | 可形成步驟, 檢核表或設計規則 |
| 新增價值 | 與現有內容重複 | 有少量補充 | 填補明確知識缺口 |

建議門檻:

- 13 至 16 分: 優先處理.
- 9 至 12 分: 視 repository 缺口決定.
- 6 至 8 分: 通常略過, 基礎教學除外.
- 0 至 5 分: 排除.

另外可以要求"一手程度"或"證據品質"至少有一項為 2 分, 避免純觀點內容靠其他項目湊分通過.

### 基礎教學的獨立通道

這套規則偏向進階工程知識, 但 repository 仍可能需要 Kubernetes, inference 或 agent loop 等基礎教材. 這類影片的講者不一定負責 production 系統, 也不一定包含事故數字, 卻可能有教學價值.
...
基礎教材不應和 production 復盤使用完全相同的評分標準, 但必須避免過時, 錯誤或過度簡化.
...
```markdown
> - 來源類型: Production owner 實戰分享
> - 一手程度: 高
> - 證據類型: 架構, 事故案例, 效能數據
> - 收錄理由: 說明 long-running agent 的失敗模式與驗證方法
> - 限制: 單一公司的內部經驗, 不一定能直接泛化
```

這比只標示頻道名稱更有用. 讀者可以立刻知道這篇筆記是產品設計的一手資料, 實戰復盤, 基礎教學, 還是經過篩選的第三方分析.
...
1. 講者是否直接做過或負責這套系統?
2. 是否提供具體系統, 案例, 數字或程式碼?
3. 是否說明失敗, 限制或取捨?
4. 內容能否轉成可執行的設計或檢核規則?
5. 相較現有筆記, 是否提供新的證據或知識?

前四題至少三個"是", 且第五題必須是"是", 才納入. 基礎教學則走獨立例外流程.

最重要的原則可以濃縮成一句:

> 優先收錄必須為事實負責的人, 並要求內容留下可驗證, 可操作, 包含限制的證據.

### 兩階段後處理流程

影片轉換與價值判斷由兩個 skill 分工:

```text
YouTube 影片
  -> knowledge-youtube-to-markdown
  -> 忠實, 可追溯的 Markdown 筆記
  -> knowledge-note-signal-review
  -> keep / skim / skip-for-now / retry-review
```

`knowledge-youtube-to-markdown` 負責忠實轉換來源, 不在生成階段預先迎合收錄偏好. `knowledge-note-signal-review` 則在筆記完成後, 根據來源責任, 證據密度, 可操作性與知識庫新增價值提出處理建議. 分開兩者可以避免篩選規則干擾內容轉換, 也保留日後重新評估的可能.

#### 審查所需資訊

審查不能只看單篇筆記, 必須同時檢查:

1. 筆記本身: 來源類型, 一手程度, 具體案例, 證據, 失敗經驗, 取捨, 適用邊界及可操作性.
2. 現有知識庫: README 與必要的既有筆記, 用來判斷內容是否重複, 填補哪個缺口, 以及是否有更可靠或更完整的來源.

邊際價值必須相對於現有收藏判斷. 一篇內容單獨看可能正確, 但若知識庫已有更完整的一手材料, 就不一定值得再次投入注意力.

#### 四種審查結果

| 結果 | 意義 | 建議處理 |
| --- | --- | --- |
| `keep` | 提供可信, 具體且有明顯新增價值的知識 | 保留於主知識庫並加入索引 |
| `skim` | 適合作為基礎或輔助材料, 但不值得深入攝取 | 保留並視需要加入索引, 不標推薦 |
| `skip-for-now` | 目前的邊際價值低, 可能抽象, 二手或高度重複 | 不加入主索引, 保留日後重評可能 |
| `retry-review` | 缺少來源身分, 內容依據或知識庫脈絡 | 補齊資訊後重新審查 |

`skip-for-now` 不是對內容做永久否定. 知識庫缺口, 技術環境或學習目標改變後, 同一份材料可能產生不同價值.

#### 審查輸出與修改邊界

審查結果應先說明決策, 信心, 來源類型與新增價值, 再列出最強證據, 重要限制, 可操作性及與既有筆記的重疊程度. 若建議保留, 還應說明適合的 README 分類及是否值得加上 `★`.

審查預設為唯讀操作. 未經明確授權, 不修改原始筆記, 不移動或刪除檔案, 不更新 README, 也不執行 commit 或 push. 這項邊界讓來源轉換保持純粹, 並避免評估工具直接替使用者做不可逆的知識整理決定.

可用以下方式執行審查:

```text
$knowledge-note-signal-review <Markdown 筆記>
```

這套流程將"已經收集"和"值得學習"分開. 生成 Markdown 只代表來源已轉成可檢查的形式, 後續審查才決定它是否值得占用注意力, 從而降低每份材料都必須立即讀完的知識焦慮.
